...
console.log('dataForUpdateBitbucket');
  conn.login(process.env.username, process.env.password, function(err, userInfo) {
    ...
    conn.query("SELECT COUNT() FROM flosum_git__History_Git__c WHERE flosum_git__isLastVersion__c = true AND flosum_git__bitbucketSync__c = true", function(err, result) {
      if(err){
        console.log('err',err);
      }else{
        console.log('result',result.totalSize);
        forAll.getFilesForCompare(records,0,result.totalSize,conn, instance).then( () => {
            ...
            conn.sobject("Flosum__Branch__c").retrieve(Array.from(branches), function(err, accounts) {
              ...
              records.forEach(function(item,index,array){
                ...
                accounts.forEach(function(acc,i,ar){
                  if(item.flosum_git__Branch_Id__c === acc.Id || acc.Id.includes(item.flosum_git__Branch_Id__c)){
              ...
              setTimeout(function(){
                bitbucket.getBitbucketFiles2(records,username,password,org).then( values => {
                ...
                //    console.log('sfResp.commit.hash',sfResp.commit.hash );
                    if(sfResp.commit != undefined){
                      ...
                      if(sfResp.commit.hash != val.values[0].commit.hash){
                        console.log('Hash',records[index].Id,sfResp.commit.hash , val.values[0].commit.hash);
                  ...
                  changedRecords.forEach(function(record,index,array){
                    if(itemsMap.get(record.flosum_git__Component__c) === undefined){
                      itemsMap.set(record.flosum_git__Component__c,[record]);
                    }else{
                      ...
                      records.forEach(function(rec,recIndex,recArray){
                        if(itemsMap.get(rec.flosum_git__Component__c) != undefined){
                          ...
                          arr.forEach(function(arRecord,arIndex,arrArray){
                            if(arRecord.flosum_git__Path__c === rec.flosum_git__Path__c){
                              exist = true;
                            }
                          });
                          if(!exist){
                  ...
                  itemsMap.forEach(function(values,key){
                    values.forEach(function(item,index,array){
                      ...
                      if(item.BitResp === undefined){
                        itemsList.push(parser.parse(item.flosum_git__Bitbucket__c).links.self.href);
                      }else{
                  ...
                  itemsList.forEach(function(o,i,a){
                    ...
                    if(i === itemsList.length-1){
                      Promise.all(contents).then(contentValues => {
                        var index = 0;
                        itemsMap.forEach(function(values,key){
                          values.forEach(function(item,index,array){
                        ...
                        itemsMap.forEach(function(value,key){
                          ...
                          value.forEach(function(o,index,array){
                            ...
                            if(o.BitResp != undefined){
                              history.flosum_git__Bitbucket__c = JSON.stringify(o.BitResp);
                            }else{
                              history.flosum_git__Bitbucket__c = JSON.stringify(o.flosum_git__Bitbucket__c); 
                            }
      
                            history.flosum_git__Branch_Id__c = o.flosum_git__Branch_Id__c;
                            GitHistoryArrTEst.push(history);
                            o.name = o.flosum_git__Path__c.split('app/main/default/')[1];
                    
                            if(type === 'CustomObject'){
                              let arr = o.name.split('/');
                              localName = arr[0]+'/'+arr[2]; 
                              name.push(localName);  
                            }else if(type === 'CustomField' || type === 'ListView' || type === 'WebLink' || type === 'FieldSet' || type === 'BusinessProcess' || type === 'CompactLayout' || type === 'SharingReason' || type === 'ValidationRule' || type === 'RecordType' ){
                              let arr = o.name.split('/');
                            ...
                            zip.file(localName,Buffer.from(content, 'ascii').toString('ascii')); 
                              });
                              zip.generateAsync({type:"base64"}).then(function (base64) {
                               ...
                               }else if((type === 'ApexClass' || value.length === 2) && type != 'AuraDefinitionBundle'){
                                  var zipData = tempZip.files[name[0].includes('.xml') ? name[1] : name[0]].asBinary();
                               ...
                               conn.sobject('Flosum__Component_History__c').create(comhis, 
                            function(err, history) {
                                ...
                                function(err, uploadedAttachment) {
                                    if(!err){
                                      ...
                                      conn.sobject('flosum_git__History_Git__c').create(GitHistoryArrTEst, 
                                        function(err, up) {
                                          console.log('TEST222');
                                            if(!err){
...
conn.login(process.env.username, process.env.password, function(err, userInfo) {
  //  conn.login('ibegei@forceoft.com.git', 'Veryeasy4473', function(err, userInfo) {
    ...
    conn.query("SELECT COUNT() FROM flosum_git__History_Git__c WHERE flosum_git__isLastVersion__c = true", function(err, result) {
      if(err){
        console.log('err',err);
      }else{
        console.log('result',result.totalSize);
  ...
  //console.log("total : " + records.length);
      Array.from(branches).forEach(function(branch,brIndex,brArray){
        ...
        forAll.httpCallSF(instanceUrl+'/services/apexrest/flosum_git/gitLab','POST',branchO,accesTok).then(resp => {
      ...
      conn.sobject("Flosum__Branch__c").retrieve(Array.from(branches), function(err, accounts) {
        ...
        setTimeout(function(){
            ...
            records.forEach(function(obj,index,array){
              ...
              contents.push(forAll.httpGet('https://gitlab.com/api/v4/projects/'+ branchWithProjId.get(brId).projectId +'/repository/files/'+ path +'?ref='+branchName,branchWithProjId.get(brId).token));
              if(index === records.length-1){
                Promise.all(contents).then(values => {
                  ...
                  changedRecords = records.filter(function(item){
                    if(item.flosum_git__GitLab__c != null || item.flosum_git__GitLab__c != undefined){
                      return item;
                    }
                  });
                }).then( () => {
                  ...
                  newMap.forEach(function(values,key){
                    values.forEach(function(item,index,array){
                      ...
                      itemsList.push(forAll.httpGet('https://gitlab.com/api/v4/projects/'+ branchWithProjId.get(brId).projectId +'/repository/files/'+ path +'?ref='+branchName,branchWithProjId.get(brId).token));
                      ii++;
                      if(ii === length){
                        Promise.all(itemsList).then(contentValues => {
                          ...
                          newMap.forEach(function(values,key){
                            values.forEach(function(item,index,array){
                              item.flosum_git__GitLab__c = contentValues[index];
                              index++;
                            });
                          });
                        })
                  ...
                  let GitHistoryArr = [];
                        newMap.forEach(function(value,key){
                          ...
                          value.forEach(function(o,index,array){
                            history = {};
                             let content = JSON.parse(o.flosum_git__GitLab__c).content.replaceAll(/\n$/,'');
                             let localName = '';
                            version = o.flosum_git__Component_Version__c + 1;
                            type = o.flosum_git__Component_type__c;
                            component = o.flosum_git__Component__c;
                  ...
                  itemsMap.forEach(function(value,key){
                    ...
                    value.forEach(function(o,index,array){
                      ...
                      if(o.BitResp =! undefined){
                        ...
                        history.flosum_git__Bitbucket__c = o.flosum_git__Bitbucket__c; 
                      }
...
app.post('/dataForUpdate', function (req, res) {
  ...
  req.body.preparedData.forEach(function(data,dataIndex,dataArray){
    if(localFilesMap.has(data.flosum_git__Branch_Id__c)){
      ...
      localFilesMap.set(data.flosum_git__Branch_Id__c,[data]);
    }
  })
  ...
  //   console.log('gitOrgSettings',gitOrgSettings);
    ...
    const uniqueArray = localFilesMap.get('a011i0000097T7XAAU').filter((thing,index) => {
      /*if(thing.flosum_git__Path__c === 'app/main/default/classes/XMLparse.cls'){
        ...
        if(index === localFilesMap.get('a011i0000097T7XAAU').findIndex(obj => {
...
console.log('reposByBranchId',reposByBranchId);
  Object.keys(reposByBranchId).forEach(function(key,item){
    ...
    let tok = gitOrgSettings.flosum_git__Git_User_Name__c + ':' + gitOrgSettings.flosum_git__Git_Password__c;
			tok = Buffer.from(tok).toString('base64');
			var tokk = 'Basic ' + tok;
    git.getGitHubBranchObjects(key, JSON.parse(reposByBranchId[key]).repo, JSON.parse(reposByBranchId[key]).sha,tokk,gitOrgSettings.flosum_git__Git_Organization__c,getRecordTypeIdbyName).then( respGit => {
        ...
        allGitObjects.forEach(function(gitObj,gitIndex,gitArr){
          if(returnGitFilesMap.has(gitObj.branchId)){
            ...
            if(allGitObjects.length-1 === gitIndex){
              git.newFiles(returnGitFilesMap,localFilesMap,gitOrgSettings,conn,reposByBranchId,getRecordTypeIdbyName);
            //   conn.login(process.env.username, process.env.password, function(err, userInfo) {
            //     let results = 0;
...
app.post('/commit', function (req, res) {
  ...
  if(req.body.mode === 'update'){
    ...
    if(branchMap.has(req.body.data.branch.Id)){
      ...
      console.log('Object.values(branchMap.get(req.body.data.branch.Id).components).length',Object.values(branchMap.get(req.body.data.branch.Id).components).length);
    }else{
    ...
    setTimeout(function(){
      if(branchMap.get(req.body.branchId).sync === 'GitHub'){
        ...
        setTimeout(function(){
          gitlab.GitLabCommit(req,branchMap.get(req.body.branchId),branchMap.get(req.body.branchId).firstReq);
          branchMap.delete(req.body.branchId);
        },10000);
      }else if(branchMap.get(req.body.branchId).sync === 'BitBucket'){
        setTimeout(function(){
            bitbucket.bitbucketPrepareCommit(req,branchMap.get(req.body.branchId),branchMap.get(req.body.branchId).firstReq);
            branchMap.delete(req.body.branchId);
        },10000);  
      }
    },5000);
